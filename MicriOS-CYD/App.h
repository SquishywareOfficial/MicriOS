...
class App {
    ...
    public:
        ...
        virtual uint16_t phaseActionColor() const;
        virtual bool wantsImmediateRender() const;
        // Immersive apps own the full panel. The shell hides its persistent
        // system bar and reveals a temporary Exit affordance on first tap.
        virtual bool prefersImmersiveMode() const;
        virtual bool handleTouch(const TouchUi::TouchSample& sample,
                                 const TouchUi::TouchEvent& event);
        virtual bool usesDefaultTouchPhaseControls() const;
    ...
    private:
        ...
        TouchUi::ControlCapture phaseTouchCapture_;
};
