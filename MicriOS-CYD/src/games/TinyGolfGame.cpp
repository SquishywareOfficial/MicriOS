...
constexpr int COURSE_Y = 39;
constexpr int COURSE_W = 300;
constexpr int COURSE_H = 140;
...
void TinyGolfGame::drawRunning(TFT_eSPI& tft) {
  CydFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    ...
    CydUi::header(canvas, "Tiny Golf", TFT_GREEN, stat.c_str());
    drawCourse(canvas);
    if (playState_ == PlayState::Aim) {
      drawAim(canvas);
      CydUi::footer(canvas, "Tap to set angle");
    } else if (playState_ == PlayState::Power) {
      drawAim(canvas);
      drawPower(canvas);
      CydUi::footer(canvas, "Tap to set power");
    } else if (playState_ == PlayState::Holed) {
      CydUi::centered(canvas, "Nice", 87, 4, TFT_YELLOW);
      CydUi::footer(canvas, "Holed");
    } else {
      CydUi::footer(canvas, "Rolling");
    }
  });
}
...
void TinyGolfGame::drawStart(TFT_eSPI& tft) {
  ...
  if (showStartPromptPage()) {
    ...
    CydUi::footer(tft, "Lowest score wins");
  } else {
    CydUi::header(tft, appTitle(), TFT_GREEN);
    tft.drawRoundRect(25, 50, 270, 118, 8, TFT_DARKGREEN);
    tft.fillCircle(63, 137, 7, TFT_WHITE);
    tft.drawCircle(265, 78, 11, TFT_DARKGREY);
    tft.fillCircle(265, 78, 5, TFT_BLACK);
    tft.fillRect(116, 51, 11, 75, TFT_DARKGREEN);
    tft.fillRect(174, 89, 92, 10, TFT_DARKGREEN);
    tft.drawLine(69, 133, 169, 91, TFT_LIGHTGREY);
    CydUi::footer(tft, "Low score wins");
  }
}

void TinyGolfGame::drawEnd(TFT_eSPI& tft) {
  CydUi::clear(tft);
  CydUi::header(tft, "Course Done", TFT_GREEN);
  CydUi::labelValue(tft, 48, "Score", String(totalStrokes_), TFT_GREEN);
  String best = "--";
  if (bestScore_ == 0) {
    best = "--";
  } else {
...
void TinyGolfGame::drawPower(Canvas& tft) {
  CydUi::bar(tft, 208, 22, 96, 10, power_, power_ > 0.78f ? TFT_RED : TFT_YELLOW);
}
