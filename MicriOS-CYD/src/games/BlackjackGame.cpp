...
void BlackjackGame::drawRunning(TFT_eSPI& tft) {
  ...
  CydFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    ...
    CydUi::header(canvas, "Blackjack", TFT_GREEN, bank.c_str());
    ...
    if (handState_ == HandState::Shuffling) {
      ...
      CydUi::centered(canvas, "Shuffling", 96, 2, TFT_YELLOW);
    ...
    if (handState_ == HandState::Betting) {
      ...
      CydUi::largeValue(canvas, "$" + String(bet_), 63, TFT_YELLOW);
      canvas.setTextSize(1);
      canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
      canvas.drawString(String(deckRemaining_) + " cards left", 86, 105);
      CydUi::footer(canvas, "Tap Deal or change the bet");
      return;
    }
    ...
    if (handState_ == HandState::Dealing) {
      CydUi::footer(canvas, "Dealing...");
    } else if (handState_ == HandState::PlayerTurn) {
      CydUi::footer(canvas, "Choose Hit or Stay");
    } else if (handState_ == HandState::DealerReveal) {
      CydUi::footer(canvas, "Dealer reveals");
    } else if (handState_ == HandState::DealerTurn) {
      CydUi::footer(canvas, handValue(dealerCards_, dealerCount_) < 17 ? "Dealer hits" : "Dealer stays");
    } else {
      CydUi::footer(canvas, resultText());
    }
  });
}
...
void BlackjackGame::drawStart(TFT_eSPI& tft) {
  ...
  if (showStartPromptPage()) {
    CydUi::header(tft, "Blackjack", TFT_GREEN);
  ...
  if (showStartScorePage()) {
    ...
    CydUi::header(tft, "Top Bank", TFT_YELLOW);
    CydUi::largeValue(tft, bestBankroll_ <= 100 ? String("--") : String("$") + String(bestBankroll_), 52, TFT_YELLOW);
    CydUi::centered(tft, bestBankroll_ <= 100 ? String("") : String(initials), 95, 2, TFT_LIGHTGREY);
    CydUi::footer(tft, "Best bankroll");
    return;
  }
  ...
  CydUi::footer(tft, "Two-deck shoe");
}

void BlackjackGame::drawEnd(TFT_eSPI& tft) {
  CydUi::clear(tft);
  CydUi::header(tft, "Cash Out", TFT_GREEN);

  CydUi::labelValue(tft, 48, "Bank", "$" + String(bankroll_), bankroll_ > 100 ? TFT_GREEN : TFT_YELLOW);
