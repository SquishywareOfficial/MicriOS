...
constexpr int CELL_W = 33;
constexpr int CELL_H = 27;
constexpr TouchUi::Rect MAZE_START = {88, 157, 144, 39};
constexpr TouchUi::Rect MAZE_RETRY = {88, 153, 144, 41};
...
bool MazeRunnerGame::handleTouch(const TouchUi::TouchSample& sample,
                                 const TouchUi::TouchEvent& event) {
  const TouchUi::Point point = TouchUi::currentPoint(sample, event);
  if (phase() == AppPhase::Start || phase() == AppPhase::End) {
    const TouchUi::Rect& button = phase() == AppPhase::Start ? MAZE_START : MAZE_RETRY;
    const TouchUi::CaptureResult result = touchCapture_.update(
        sample, event, button.contains(point) ? 1 : TouchUi::NO_CONTROL);
    if (result.activated) startRunning();
    return result.consumed;
  }
  if (runnerState_ != RunnerState::Choosing || optionCount_ == 0) return false;

  const int runnerX = GRID_X + (runnerCell_ % COLS) * CELL_W + CELL_W / 2;
  const int runnerY = GRID_Y + (runnerCell_ / COLS) * CELL_H + CELL_H / 2;
  int16_t hit = TouchUi::NO_CONTROL;
  for (uint8_t i = 0; i < optionCount_; ++i) {
    int x = runnerX;
    int y = runnerY;
    if (options_[i] == Up) y -= CELL_H;
    else if (options_[i] == Right) x += CELL_W;
    else if (options_[i] == Down) y += CELL_H;
    else x -= CELL_W;
    if (TouchUi::Rect{static_cast<int16_t>(x - 15), static_cast<int16_t>(y - 13), 30, 26}.contains(point)) {
      hit = i;
      break;
    }
  }
  const TouchUi::CaptureResult result = touchCapture_.update(sample, event, hit);
  if (result.activated && result.id >= 0 && result.id < optionCount_) {
    selectedOption_ = static_cast<uint8_t>(result.id);
    chooseDirection();
  }
  return result.consumed;
}
...
void MazeRunnerGame::drawRunning(TFT_eSPI& tft) {
  CydFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    ...
    CydUi::header(canvas, collectorMode_ ? "Maze Collector" : "Maze Runner", collectorMode_ ? TFT_YELLOW : TFT_CYAN, stat.c_str());
    ...
    if (collectorMode_) {
      for (uint8_t i = 0; i < pickupCount_; i++) {
        ...
        canvas.fillCircle(pickupX, pickupY, 6, TFT_YELLOW);
        canvas.drawCircle(pickupX, pickupY, 7, TFT_ORANGE);
      }
    }
    canvas.fillCircle(runnerX, runnerY, 7, TFT_GREEN);
    canvas.fillCircle(runnerX + 2, runnerY - 2, 1, TFT_BLACK);
    if (runnerState_ == RunnerState::Choosing && optionCount_ > 0) {
      for (uint8_t optionIndex = 0; optionIndex < optionCount_; ++optionIndex) {
      const Direction option = options_[optionIndex];
      int x2 = runnerX;
      int y2 = runnerY;
      if (option == Up) {
        y2 -= CELL_H;
      ...
      canvas.fillCircle(x2, y2, 5, optionIndex == selectedOption_ ? TFT_YELLOW : TFT_CYAN);
      }
    }
    const int exitX = GRID_X + (COLS - 1) * CELL_W + CELL_W / 2 - 8;
    const int exitY = GRID_Y + (ROWS - 1) * CELL_H + CELL_H / 2 - 8;
    canvas.drawRoundRect(exitX, exitY, 16, 16, 3, exitUnlocked() ? TFT_GREEN : TFT_RED);
    if (collectorMode_ && !exitUnlocked()) {
      canvas.drawLine(exitX + 3, exitY + 3, exitX + 13, exitY + 13, TFT_RED);
      canvas.drawLine(exitX + 13, exitY + 3, exitX + 3, exitY + 13, TFT_RED);
    }
    CydUi::footer(canvas, runnerState_ == RunnerState::Choosing ? "Tap a direction" : "Running...");
  });
}
...
void MazeRunnerGame::drawStart(TFT_eSPI& tft) {
  ...
  if (showStartPromptPage()) {
    CydUi::header(tft, appTitle(), collectorMode_ ? TFT_YELLOW : TFT_CYAN);
    CydUi::centered(tft, "Press", 50, 3, TFT_WHITE);
    CydUi::centered(tft, "to Start", 78, 2, TFT_LIGHTGREY);
    CydUi::touchAction(tft, MAZE_START, "Start", TFT_GREEN);
  } else if (showStartScorePage()) {
    char initials[4];
    PlayerProfile::unpackDottedInitials(bestInitials_, initials);

    CydUi::header(tft, "Top Level", TFT_YELLOW);
    CydUi::largeValue(tft, bestLevel_ == 0 ? String("--") : "L" + String(bestLevel_), 54, TFT_YELLOW);
    CydUi::centered(tft, bestLevel_ == 0 ? String("") : String(initials), 96, 2, TFT_LIGHTGREY);
    CydUi::touchAction(tft, MAZE_START, "Start", TFT_GREEN);
  } else {
    CydUi::header(tft, appTitle(), collectorMode_ ? TFT_YELLOW : TFT_CYAN);
    for (uint8_t x = 0; x < 6; x++) {
      tft.drawRect(49 + x * 37, 57, 37, 30, TFT_DARKGREY);
      if (x != 2) tft.drawFastVLine(49 + x * 37, 57, 30, TFT_LIGHTGREY);
    }
    tft.fillCircle(104, 72, 7, TFT_GREEN);
    if (collectorMode_) tft.fillCircle(178, 72, 6, TFT_YELLOW);
    CydUi::centered(tft, collectorMode_ ? "Collect keys" : "Choose paths", 112, 2, TFT_LIGHTGREY);
    CydUi::touchAction(tft, MAZE_START, "Start", TFT_GREEN);
  }
}
...
void MazeRunnerGame::drawEnd(TFT_eSPI& tft) {
  CydUi::clear(tft);
  CydUi::header(tft, "Lost", TFT_RED);
  CydUi::labelValue(tft, 48, "Level", String(level_), TFT_CYAN);
  ...
  CydUi::touchAction(tft, MAZE_RETRY, "Play Again", TFT_GREEN);
}
