...
void MicriFieldGame::updateRunning(uint32_t deltaMs, const ButtonInput& b1, const ButtonInput& b2) {
  ...
  if (state_ == FieldState::HurdleCue) {
    ...
    // Jumping trades speed for clearance. An unnecessarily early jump therefore
    // costs time even if the athlete still clears the hurdle.
    hurdleX_ -= hurdleSpeed_ * dt * (hurdleRunnerY_ > 0.1f ? 0.82f : 1.0f);
    const float runnerX = 55.0f;
...
void MicriFieldGame::setFieldLed(bool on) {
  CydHardware::setRgb(0, on ? 255 : 0, on ? 48 : 0);
}

void MicriFieldGame::onAppExit() { setFieldLed(false); }
...
void MicriFieldGame::drawStart(TFT_eSPI& tft) {
  ...
  if (showStartScorePage()) {
    ...
    CydUi::header(tft, "Top Total", TFT_YELLOW);
    CydUi::largeValue(tft, bestTotalScore_ == 0 ? String("--") : String(bestTotalScore_), 47, TFT_YELLOW);
    CydUi::centered(tft, bestTotalScore_ == 0 ? String("") : String(initials), 86, 2, TFT_LIGHTGREY);
    tft.setTextSize(1);
    tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    tft.drawString(String(eventName(eventRecord)) + " best " + String(bestEventScores_[eventRecord]), 12, 145);
    CydUi::footer(tft, "Event records rotate");
    return;
  }
  drawTrackSplash(tft);
}
...
void MicriFieldGame::drawEnd(TFT_eSPI& tft) {
  CydUi::clear(tft);
  setFieldLed(false);
  CydUi::header(tft, "Game Over", TFT_RED);
  CydUi::labelValue(tft, 48, "Score", String(totalScore_), TFT_ORANGE);
  CydUi::labelValue(tft, 78, "Best", String(bestTotalScore_), TFT_YELLOW);
...
void MicriFieldGame::drawScoreHeader(Canvas& tft) {
  ...
  CydUi::header(tft, title.c_str(), TFT_ORANGE, target.c_str());
}
...
void MicriFieldGame::drawEventIntro(Canvas& tft) {
  ...
  CydUi::centered(tft, eventName(eventIndex_), 59, 4, TFT_WHITE);
  CydUi::labelValue(tft, 116, "Qual", formatEventValue(eventIndex_, qualifyingTarget()), TFT_YELLOW);
  tft.setTextSize(1);
  tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  if (pressureMode_) {
    tft.drawString("Pressure mode: 1 try", 12, 151);
  } else {
    tft.drawString("Attempts left " + String(attemptsRemaining_), 12, 151);
  }
  CydUi::footer(tft, "Tap to start event");
}
...
void MicriFieldGame::drawCueSequence(Canvas& tft) {
  ...
  CydUi::largeValue(tft, push ? String("PUSH") : String("WAIT"), 60, push ? TFT_GREEN : TFT_YELLOW);
  if (messageTimerMs_ > 0) {
    CydUi::centered(tft, "Early x2 penalty", 123, 2, TFT_RED);
  } else {
    CydUi::centered(tft, "Quality " + String(lastQuality_), 123, 2, TFT_LIGHTGREY);
  }
  CydUi::bar(tft, 58, 157, 244, 10, cueTotal_ == 0 ? 0.0f : static_cast<float>(cueDone_) / static_cast<float>(cueTotal_), TFT_CYAN);
  if (cueMode_ == CueMode::LongRun || cueMode_ == CueMode::JavelinRun || cueMode_ == CueMode::Dash) {
    drawStick(tft, 28 + (cueDone_ * 25), 174, false);
  }
}
...
void MicriFieldGame::drawHurdles(Canvas& tft) {
  ...
  CydUi::footer(tft, hurdleFooter.c_str());
}
...
void MicriFieldGame::drawDirectionSelect(Canvas& tft) {
  ...
  tft.drawCircle(cx, cy, 66, TFT_DARKGREY);
  const float rad = directionDeg_ * 0.0174533f;
  const int bx = cx + static_cast<int>(cosf(rad) * 66);
  const int by = cy + static_cast<int>(sinf(rad) * 66);
  tft.drawLine(cx, cy, bx, by, TFT_YELLOW);
  tft.fillCircle(bx, by, 5, TFT_YELLOW);
  tft.drawLine(cx, cy, cx + 82, cy, TFT_GREEN);

  CydUi::centered(tft, "Aim ahead", 42, 2, TFT_LIGHTGREY);
  CydUi::footer(tft, "Tap to choose throw direction");
}

template <typename Canvas>
void MicriFieldGame::drawAngleSelect(Canvas& tft) {
  drawScoreHeader(tft);
  drawAngleFan(tft, 48, 170, selectorAngle_);

  CydUi::centered(tft, "ANGLE", 49, 2, TFT_YELLOW);
  CydUi::largeValue(tft, String(static_cast<int>(selectorAngle_)) + "deg", 80, TFT_WHITE);
  CydUi::footer(tft, "Tap to select angle");
}
...
void MicriFieldGame::drawJavelinHoldAngle(Canvas& tft) {
  drawScoreHeader(tft);
  drawAngleFan(tft, 48, 170, selectorAngle_);
  tft.drawLine(29, 172, 169, 119, TFT_LIGHTGREY);

  CydUi::centered(tft, holding_ ? "Release" : "Hold", 49, 2, holding_ ? TFT_GREEN : TFT_YELLOW);
  CydUi::largeValue(tft, String(static_cast<int>(selectorAngle_)) + "deg", 80, TFT_WHITE);
  CydUi::footer(tft, "Hold for javelin angle");
}
...
void MicriFieldGame::drawHighJumpHold(Canvas& tft) {
  ...
  const int ground = 174;
  tft.drawFastHLine(0, ground, width, TFT_DARKGREEN);
  tft.drawLine(222, 78, 222, ground, TFT_LIGHTGREY);
  tft.drawLine(275, 78, 275, ground, TFT_LIGHTGREY);
  tft.drawLine(222, 78, 275, 78, TFT_YELLOW);
  const int jumpY = holding_ ? ground - min<int>(72, highHoldMs_ / 11) : ground;
  drawStick(tft, 92, jumpY, holding_);
  CydUi::centered(tft, holding_ ? "Release" : "Hold", 49, 2, holding_ ? TFT_GREEN : TFT_YELLOW);
  CydUi::footer(tft, "Hold to set jump angle");
}
...
void MicriFieldGame::drawResultAnim(Canvas& tft) {
  ...
  const int groundY = 174;
  const String target = "Qual " + formatEventValue(eventIndex_, qualifyingTarget());
  CydUi::header(tft, eventName(eventIndex_), TFT_ORANGE, target.c_str());

  if (eventIndex_ == 0) {
    const int finishX = 292;
    const int laneGround[3] = {78, 124, 172};
    for (uint8_t lane = 0; lane < 3; ++lane) {
      tft.drawFastHLine(0, laneGround[lane] + 1, width, TFT_DARKGREY);
    }
    for (int y = 39; y < groundY; y += 10) {
      tft.fillRect(finishX, y, 4, 5, ((y / 5) & 1) ? TFT_WHITE : TFT_DARKGREY);
    }
    ...
    for (uint8_t lane = 0; lane < 3; ++lane) {
      const int runnerX = 20 + static_cast<int>(272.0f * min(1.0f, progress * pace[lane]));
      drawStick(tft, runnerX, laneGround[lane], false, colors[lane], gait + lane);
    }
    CydUi::footer(tft, ("You " + formatEventValue(0, resultValue_)).c_str());
    return;
  }

  tft.drawFastHLine(0, groundY, width, TFT_DARKGREEN);
  tft.drawFastHLine(0, groundY + 1, width, TFT_DARKGREEN);

  if (eventIndex_ == 2) {
    const int takeoffX = 118;
    const float ratio = clampFloat(static_cast<float>(resultValue_) /
                                       static_cast<float>(qualifyingTarget()),
                                   0.55f, 1.35f);
    const int landingX = min<int>(302, takeoffX + static_cast<int>(125.0f * ratio));
    tft.fillRect(takeoffX, groundY - 5, 5, 7, TFT_WHITE);
    tft.fillRect(takeoffX + 7, groundY + 2, width - takeoffX - 14, 7, TFT_YELLOW);
    ...
    const int athleteX = progress <= 0.28f
                             ? 28 + static_cast<int>((takeoffX - 28) * runP)
                             : takeoffX + static_cast<int>((landingX - takeoffX) * flightP);
    const int lift = progress <= 0.28f ? 0 : static_cast<int>(sinf(flightP * PI) * 58.0f);
    drawStick(tft, athleteX, groundY - lift, lift > 2, TFT_GREEN, gait);
    tft.drawFastVLine(landingX, groundY - 12, 12, TFT_CYAN);
    CydUi::footer(tft, ("Distance " + formatEventValue(2, resultValue_)).c_str());
    return;
  }

  if (eventIndex_ == 3) {
    const int originX = 62;
  ...
  if (eventIndex_ == 4) {
    const int originX = 58;
    drawStick(tft, originX - 8, groundY, false, TFT_CYAN, gait);
    const float ratio = clampFloat(static_cast<float>(resultValue_) /
                                       static_cast<float>(qualifyingTarget()),
                                   0.55f, 1.35f);
    const int travel = static_cast<int>(230.0f * min(1.0f, ratio));
    const int spearX = originX + static_cast<int>(travel * progress);
    const int spearY = groundY - 23 - static_cast<int>(sinf(progress * PI) * 70.0f);
    tft.drawLine(spearX - 29, spearY + 7, spearX + 8, spearY - 2, TFT_YELLOW);
    tft.fillTriangle(spearX + 8, spearY - 2, spearX + 1, spearY - 5,
                     spearX + 2, spearY + 2, TFT_WHITE);
    CydUi::footer(tft, ("Distance " + formatEventValue(4, resultValue_)).c_str());
    return;
  }

  const int leftPost = 170;
  const int rightPost = 225;
  const int barY = 91;
  ...
  if (barFallen) {
    tft.drawLine(leftPost, barY + 4, rightPost, groundY - 8, TFT_RED);
  } else {
    tft.drawFastHLine(leftPost, barY, rightPost - leftPost, TFT_YELLOW);
  }
  const int athleteX = 48 + static_cast<int>(215.0f * progress);
  const int peak = qualified_ ? 89 : 62;
  ...
  CydUi::footer(tft, (String(qualified_ ? "Cleared " : "Missed ") +
...
void MicriFieldGame::drawResult(Canvas& tft) {
  ...
  CydUi::header(tft, qualified_ ? "Qualified" : "No Qual",
  ...
  CydUi::labelValue(tft, 55, resultUnit_ == ResultUnit::Time ? "Time" : (resultUnit_ == ResultUnit::Height ? "Height" : "Dist"), value, TFT_YELLOW);
  CydUi::labelValue(tft, 95, "Points", String(resultScore_), TFT_GREEN);
  CydUi::labelValue(tft, 135, "Total", String(totalScore_), TFT_ORANGE);
  if (qualified_) {
    CydUi::footer(tft, "Tap for next event");
  } else if (!pressureMode_ && attemptsRemaining_ > 0) {
    CydUi::footer(tft, ("Retry - attempts " + String(attemptsRemaining_)).c_str());
  } else {
    CydUi::footer(tft, "Continue - 0 points");
  }
}
...
void MicriFieldGame::drawStick(Canvas& tft, int x, int groundY, bool airborne,
  ...
  const int stride = (gait & 1U) ? 11 : -11;
  tft.fillCircle(x, y - 8, 5, TFT_ORANGE);
  tft.drawCircle(x, y - 8, 5, TFT_WHITE);
  tft.fillRoundRect(x - 3, y - 2, 7, 15, 2, color);
  tft.fillRect(x - 4, y + 11, 9, 5, TFT_BLUE);
  const int armLift = airborne ? -4 : ((gait & 1U) ? 4 : -1);
  tft.drawLine(x - 2, y + 2, x + 12, y + armLift, color);
  tft.drawLine(x + 2, y + 3, x - 11, y + 8 - armLift, color);
  tft.drawLine(x - 2, y + 15, x + stride, groundY - (airborne ? 9 : 0), TFT_WHITE);
  tft.drawLine(x + 2, y + 15, x - stride, groundY - (airborne ? 14 : 0), TFT_WHITE);
  tft.drawLine(x + stride, groundY - (airborne ? 9 : 0),
               x + stride + 5, groundY - (airborne ? 8 : 0), TFT_WHITE);
}
...
void MicriFieldGame::drawAngleFan(Canvas& tft, int ox, int oy, float angleDeg) {
  tft.drawLine(ox, oy, ox + 218, oy, TFT_DARKGREY);
  tft.drawLine(ox, oy, ox + 154, oy - 113, TFT_DARKGREY);
  tft.drawArc(ox, oy, 88, 85, 315, 360, TFT_DARKGREY, TFT_BLACK);
  const float rad = -angleDeg * 0.0174533f;
  const int x2 = ox + static_cast<int>(cosf(rad) * 184);
  const int y2 = oy + static_cast<int>(sinf(rad) * 184);
  tft.drawLine(ox, oy, x2, y2, TFT_YELLOW);
  tft.fillCircle(x2, y2, 5, TFT_YELLOW);
}

template <typename Canvas>
void MicriFieldGame::drawTrackSplash(Canvas& tft) {
  CydUi::header(tft, "Micri Field", TFT_ORANGE);
  CydUi::centered(tft, "Track & Field", 43, 2, TFT_LIGHTGREY);
  tft.drawFastHLine(0, 171, width, TFT_DARKGREEN);
  tft.drawFastHLine(0, 180, width, TFT_DARKGREEN);
  drawStick(tft, 68, 171, false);
  drawHurdle(tft, 137, 171);
  tft.drawLine(218, 122, 281, 73, TFT_YELLOW);
  tft.fillCircle(288, 69, 5, TFT_LIGHTGREY);
  CydUi::footer(tft, "Six arcade events");
}
