...
class CaveChopperGame : public App {
  ...
  protected:
    ...
    uint16_t phaseActionColor() const override;

  private:
    static constexpr uint8_t SEGMENTS = 34;
    static constexpr uint8_t SEG_W = 10;
    static constexpr uint8_t GAP_H = 76;

    struct Segment {
      float x = 0.0f;
      int gapTop = 0;
      uint8_t obstacleDepth = 0;
      bool obstacleFromTop = false;
      bool active = false;
    };
    ...
    int lastGapTop_ = 32;
    uint8_t obstacleCooldown_ = 0;
    uint8_t obstacleRemaining_ = 0;
    uint8_t obstacleSpan_ = 0;
    bool obstacleFromTop_ = false;
    bool initializingSegments_ = false;
