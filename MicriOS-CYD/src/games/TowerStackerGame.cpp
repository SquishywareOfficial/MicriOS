...
void TowerStackerGame::drawRunning(TFT_eSPI& tft) {
  CydFramebuffer::draw(tft, static_cast<int16_t>(width), static_cast<int16_t>(height), [&](auto& canvas) {
    CydUi::clear(canvas);
    CydUi::header(canvas, "Tower Stacker", TFT_CYAN, (String("L") + String(level_) + " S" + String(score_)).c_str());
...
void TowerStackerGame::drawStart(TFT_eSPI& tft) { CydUi::clear(tft);
  loadHighScore();
  CydUi::clear(tft);
  if (showStartPromptPage()) {
    CydUi::header(tft, "Tower Stacker", TFT_CYAN);
    CydUi::centered(tft, "Press to Start", 56, 2, TFT_WHITE);
    CydUi::footer(tft, "Tap to drop the moving block");
  } else if (showStartScorePage()) {
    char initials[4];
    PlayerProfile::unpackDottedInitials(highScoreInitials_, initials);
    CydUi::header(tft, "Top Score", TFT_CYAN);
    String score = highScore_ == 0 ? "--" : String(initials) + " " + String(highScore_);
    CydUi::largeValue(tft, score, 54, TFT_CYAN);
    CydUi::footer(tft, "Total stacked blocks");
  } else {
    CydUi::header(tft, "Tower Stacker", TFT_CYAN);
    tft.fillRect(141, 54, 38, 86, TFT_BLUE);
    tft.fillRect(118, 82, 84, 58, TFT_CYAN);
    tft.fillRect(88, 112, 144, 28, TFT_YELLOW);
    tft.drawFastHLine(60, 141, 200, TFT_LIGHTGREY);
    CydUi::centered(tft, "Stack clean", 158, 2, TFT_LIGHTGREY);
  }
}

void TowerStackerGame::drawEnd(TFT_eSPI& tft) { CydUi::clear(tft);
  CydUi::clear(tft);
