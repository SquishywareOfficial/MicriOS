...
constexpr float BOARD_SCALE = 3.35f;
constexpr int BOARD_CX = 215;
constexpr int BOARD_CY = 109;
}
...
void KnifeThrowGame::drawRunning(TFT_eSPI& tft) {
  CydFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "Knife Throw", TFT_ORANGE, (String("S") + String(score_)).c_str());
    drawBoard(canvas, BOARD_CX, BOARD_CY);
    CydUi::footer(canvas, throwActive_ ? "Knife in flight..." : "Tap to throw");
  });
}

void KnifeThrowGame::drawStart(TFT_eSPI& tft) { CydUi::clear(tft);
  loadBest();
  CydUi::clear(tft);
  if (showStartPromptPage()) {
    CydUi::header(tft, "Knife Throw", TFT_ORANGE);
    CydUi::centered(tft, "Press to Start", 56, 2, TFT_WHITE);
    CydUi::footer(tft, "Do not hit the person");
  } else if (showStartScorePage()) {
    char initials[4];
    PlayerProfile::unpackDottedInitials(bestInitials_, initials);
    CydUi::header(tft, "Top Score", TFT_ORANGE);
    String score = bestScore_ == 0 ? "--" : String(initials) + " " + String(bestScore_);
    CydUi::largeValue(tft, score, 54, TFT_ORANGE);
    CydUi::footer(tft, "Best clean throws");
  } else {
    CydUi::header(tft, "Knife Throw", TFT_ORANGE);
    drawBoard(tft, BOARD_CX, BOARD_CY);
    tft.drawLine(24, 174, 88, 144, TFT_WHITE);
    tft.drawLine(88, 144, 102, 147, TFT_LIGHTGREY);
    CydUi::footer(tft, "Time the spinning board");
  }
}

void KnifeThrowGame::drawEnd(TFT_eSPI& tft) { CydUi::clear(tft);
  CydUi::clear(tft);
  CydUi::header(tft, hitPerson_ ? "Hit Person" : "Hit Knife", TFT_RED);
  CydUi::labelValue(tft, 49, "Score", String(score_), TFT_YELLOW);
