...
constexpr int CELL = 27;
constexpr int PANEL_X = 224;
constexpr TouchUi::Rect PIPE_PIECE = {218, 57, 92, 48};
constexpr TouchUi::Rect PIPE_START = {88, 157, 144, 39};
constexpr TouchUi::Rect PIPE_RETRY = {88, 153, 144, 41};
...
bool PipeManiaGame::handleTouch(const TouchUi::TouchSample& sample,
  ...
  if (phase() == AppPhase::Start) {
    if (PIPE_START.contains(point)) hit = 1;
  } else if (phase() == AppPhase::End) {
    if (PIPE_RETRY.contains(point)) hit = 2;
  } else if (pipeState_ == PipeState::Build) {
    if (PIPE_PIECE.contains(point)) hit = 3;
    if (cursor_ < CELL_COUNT) {
      const int x = GRID_X + (cursor_ % COLS) * CELL;
      const int y = GRID_Y + (cursor_ / COLS) * CELL;
      if (TouchUi::Rect{static_cast<int16_t>(x - 4), static_cast<int16_t>(y - 4),
                        static_cast<int16_t>(CELL + 8), static_cast<int16_t>(CELL + 8)}.contains(point)) hit = 4;
    }
  }
  const TouchUi::CaptureResult result = touchCapture_.update(sample, event, hit);
  if (!result.activated) return result.consumed;
  if (result.id == 1 || result.id == 2) startRunning();
  else if (result.id == 3) cyclePiece();
  else if (result.id == 4) placePiece();
  return true;
}
...
void PipeManiaGame::drawRunning(TFT_eSPI& tft) {
  CydFramebuffer::draw(tft, width, height, [&](auto& canvas) {
    ...
    CydUi::header(canvas, "Pipe Mania", TFT_GREEN, stat.c_str());
    ...
    canvas.drawString("Next", PANEL_X, 43);
    canvas.drawRoundRect(PIPE_PIECE.x, PIPE_PIECE.y, PIPE_PIECE.w, PIPE_PIECE.h, 5, TFT_CYAN);
    drawPipe(canvas, PANEL_X + 4, 67, currentPiece_, false);
    canvas.setTextSize(1);
    canvas.setTextColor(TFT_CYAN, TFT_BLACK);
    canvas.drawString("Tap to change", PANEL_X, 108);
    canvas.setTextSize(2);
    if (pipeState_ == PipeState::Build) {
      if (cursor_ >= CELL_COUNT) {
        canvas.setTextColor(TFT_RED, TFT_BLACK);
        canvas.drawString("Blocked", PANEL_X, 116);
      } else {
        canvas.setTextColor(TFT_YELLOW, TFT_BLACK);
        canvas.drawString(String((buildDelayMs() - min(buildTimerMs_, buildDelayMs())) / 1000) + "s", PANEL_X, 116);
      }
    } else if (pipeState_ == PipeState::Complete) {
      canvas.setTextColor(TFT_GREEN, TFT_BLACK);
      canvas.drawString("Clear", PANEL_X, 116);
    } else {
      canvas.setTextColor(TFT_GREEN, TFT_BLACK);
      canvas.drawString("Goo", PANEL_X, 116);
    }
    CydUi::footer(canvas, pipeState_ == PipeState::Build ? "Tap highlighted cell to place" : "Flowing...");
  });
}
...
void PipeManiaGame::drawStart(TFT_eSPI& tft) {
  ...
  if (showStartPromptPage()) {
    ...
    CydUi::centered(tft, "Outrun the goo", 88, 2, TFT_LIGHTGREY);
    CydUi::touchAction(tft, PIPE_START, "Start", TFT_GREEN);
  }
}
...
void PipeManiaGame::drawEnd(TFT_eSPI& tft) {
  CydUi::clear(tft);
  CydUi::header(tft, won_ ? "Pipe Master" : "Pipe End", won_ ? TFT_GREEN : TFT_RED);
  CydUi::labelValue(tft, 48, "Score", String(score_), TFT_GREEN);
  ...
  CydUi::touchAction(tft, PIPE_RETRY, "Play Again", TFT_GREEN);
}
