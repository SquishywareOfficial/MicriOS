...
void drawInfoLine(Canvas& canvas, int y, const char* label, const String& value, uint16_t color = TFT_WHITE) {
  canvas.fillRect(12, y - 2, CydUi::W - 24, 16, TFT_BLACK);
  canvas.setTextDatum(TL_DATUM);
  ...
  const int maxWidth = CydUi::W - 96;
  while (clipped.length() > 0 && canvas.textWidth(clipped) > maxWidth) clipped.remove(clipped.length() - 1);
  canvas.drawString(clipped, 86, y);
}
...
void drawCenteredValueBand(Canvas& canvas,
                           ...
                           uint8_t maxSize,
                           uint16_t color) {
  canvas.fillRect(0, y, CydUi::W, h, TFT_BLACK);
  uint8_t size = maxSize;
  canvas.setTextDatum(TL_DATUM);
  do {
    canvas.setTextSize(size);
    if (canvas.textWidth(text) <= CydUi::W - CydUi::PAD * 2 || size <= 2) {
      break;
    }
    size--;
  } while (true);
  canvas.setTextColor(color, TFT_BLACK);
  canvas.drawString(text, (CydUi::W - canvas.textWidth(text)) / 2, y + 2);
}
}

MicriMinerApp::MicriMinerApp(uint32_t width, uint32_t height)
    : App("Micri Miner", width, height), server_(80), apIP_(AP_IP) {}

bool MicriMinerApp::hasCustomOverlay() const {
  return true;
}

bool MicriMinerApp::startsRunningImmediately() const {
  return true;
}

uint16_t MicriMinerApp::runningRenderIntervalMs() const {
  return mode_ == Mode::Portal ? 500 : 750;
}

bool MicriMinerApp::wantsImmediateRender() const {
  return dirty_;
}

void MicriMinerApp::setDebugAutoStart(bool enabled) {
  debugAutoStart_ = enabled;
}

void MicriMinerApp::setEmitDebugLogs(bool enabled) {
  emitDebugLogs_ = enabled;
}

bool MicriMinerApp::emitDebugLogs() const {
  return emitDebugLogs_;
}

void MicriMinerApp::debugStartMining() {
  if (!miner_.running()) {
...
void MicriMinerApp::debugStopMining() {
  if (miner_.running()) {
...
void MicriMinerApp::debugPrintStats(const char* reason) {
  printStatsLine(miner_.stats(), reason);
}

void MicriMinerApp::markDirty() {
  dirty_ = true;
}

void MicriMinerApp::forceClear() {
  dirty_ = true;
  forceScreenClear_ = true;
}

void MicriMinerApp::onAppReset() {
...
void MicriMinerApp::onAppExit() {
  persistRuntimeTotals(true);
  stopPortal();
  miner_.stop();
}

void MicriMinerApp::launchPortal() {
  persistRuntimeTotals(true);
  miner_.stop();
...
void MicriMinerApp::startPortal() {
  portalLaunchPending_ = false;
  stopPortal();
...
void MicriMinerApp::setupRoutes() {
...
void MicriMinerApp::handleRoot() {
  server_.send(200, "text/html", buildPage(nullptr));
}

void MicriMinerApp::handleSave() {
  ...
  server_.send(200, "text/html", buildPage("Saved. Return to MicriOS."));
  markDirty();
}

void MicriMinerApp::handleReset() {
  settings_.reset();
  server_.send(200, "text/html", buildPage("Miner settings reset."));
  markDirty();
}

void MicriMinerApp::handleNotFound() {
  server_.sendHeader("Location", "/", true);
  server_.send(302, "text/plain", "");
}

String MicriMinerApp::escapeHtml(const String& value) const {
  String out;
...
void MicriMinerApp::loadLifetimeStats() {
  Preferences prefs;
...
void MicriMinerApp::saveLifetimeStats() {
  Preferences prefs;
...
void MicriMinerApp::resetSessionPersistMarkers() {
...
void MicriMinerApp::persistRuntimeTotals(bool force) {
...
bool MicriMinerApp::connectWifiForPrice() {
...
bool MicriMinerApp::fetchBtcPrice() {
...
String MicriMinerApp::formatHashCount(uint64_t value) const {
...
String MicriMinerApp::formatDuration(uint64_t seconds) const {
...
String MicriMinerApp::buildPage(const char* notice) const {
  ...
  html += F("<title>Micri Miner Setup</title><style>body{font-family:sans-serif;margin:24px;max-width:640px}input{box-sizing:border-box;width:100%;padding:10px;margin:6px 0 14px}button{padding:10px 14px;margin-right:8px}.note{color:#075}.muted{color:#666}</style></head><body>");
  html += F("<h1>Micri Miner Setup</h1>");
  html += F("<p class='muted'>Configure pool and payout wallet. WiFi networks are still configured from MicriOS WiFi Setup.</p>");
  if (notice != nullptr) {
    html += F("<p class='note'>");
    html += notice;
    html += F("</p>");
...
void MicriMinerApp::showMessage(const char* message) {
  message_ = message;
  mode_ = Mode::Message;
  modeStartedAtMs_ = millis();
  forceClear();
...
namespace {
constexpr TouchUi::Rect MINER_PREV = {8, 174, 52, 28};
constexpr TouchUi::Rect MINER_ACTION = {66, 166, 188, 36};
constexpr TouchUi::Rect MINER_NEXT = {260, 174, 52, 28};
constexpr TouchUi::Rect MINER_CANCEL = {20, 151, 132, 43};
constexpr TouchUi::Rect MINER_CONFIRM = {168, 151, 132, 43};
constexpr TouchUi::Rect MINER_CONTINUE = {90, 151, 140, 43};
}

bool MicriMinerApp::handleTouch(const TouchUi::TouchSample& sample,
                                const TouchUi::TouchEvent& event) {
  const TouchUi::Point point = TouchUi::currentPoint(sample, event);
  int16_t hit = TouchUi::NO_CONTROL;
  if (mode_ == Mode::Dashboard) {
    if (MINER_PREV.contains(point)) hit = 0;
    if (MINER_ACTION.contains(point)) hit = 1;
    if (MINER_NEXT.contains(point)) hit = 2;
  } else if (mode_ == Mode::StartingPortal || mode_ == Mode::Portal ||
             mode_ == Mode::Message) {
    if (MINER_CONTINUE.contains(point)) hit = 0;
  } else if (mode_ == Mode::ConfirmReset) {
    if (MINER_CANCEL.contains(point)) hit = 0;
    if (MINER_CONFIRM.contains(point)) hit = 1;
  }

  const auto result = touchCapture_.update(sample, event, hit);
  if (result.pressed || result.released) markDirty();
  if (!result.activated) {
    if (mode_ == Mode::Dashboard && event.released &&
        (event.swipe == TouchUi::SwipeDirection::Left ||
         event.swipe == TouchUi::SwipeDirection::Right)) {
      int16_t page = static_cast<int16_t>(page_);
      page += event.swipe == TouchUi::SwipeDirection::Left ? 1 : -1;
      if (page < 0) page = static_cast<int16_t>(Page::Count) - 1;
      if (page >= static_cast<int16_t>(Page::Count)) page = 0;
      page_ = static_cast<Page>(page);
      if (page_ == Page::Price && priceIsStale(millis())) queueBtcPriceFetch();
      else forceClear();
      return true;
    }
  ...
  if (mode_ == Mode::StartingPortal || mode_ == Mode::Portal) {
  ...
  } else if (result.id == 0 || result.id == 2) {
    int16_t page = static_cast<int16_t>(page_);
    page += result.id == 2 ? 1 : -1;
    if (page < 0) page = static_cast<int16_t>(Page::Count) - 1;
    if (page >= static_cast<int16_t>(Page::Count)) page = 0;
    page_ = static_cast<Page>(page);
    if (page_ == Page::Price && priceIsStale(millis())) queueBtcPriceFetch();
    ...
    forceClear();
  } else if (page_ == Page::Price) {
    fetchBtcPrice();
    forceClear();
  } else if (miner_.running()) {
    persistRuntimeTotals(true);
    miner_.stop();
    forceClear();
  } else {
    if (priceIsStale(millis())) fetchBtcPrice();
    resetSessionPersistMarkers();
    miner_.start(HOSTNAME);
    forceClear();
  }
...
void MicriMinerApp::updateRunning(uint32_t deltaMs, const ButtonInput& b1, const ButtonInput& b2) {
  (void)deltaMs;
...
void MicriMinerApp::maybePrintSerialStats(uint32_t nowMs) {
...
void MicriMinerApp::printStatsLine(const MinerLogic::Stats& stats, const char* reason) {
...
const char* MicriMinerApp::pageTitle() const {
  switch (page_) {
    ...
    case Page::Dashboard:
    default: return "Micri Miner";
  }
}

uint16_t MicriMinerApp::stateColor(MinerLogic::State state) const {
  switch (state) {
...
template <typename Canvas>
void MicriMinerApp::drawPageControls(Canvas& canvas, const char* action,
                                     uint16_t actionColor) {
  CydUi::touchAction(canvas, MINER_PREV, "<", TFT_BLUE, false,
                     touchCapture_.activeId() == 0);
  CydUi::touchAction(canvas, MINER_ACTION, action, actionColor, true,
                     touchCapture_.activeId() == 1);
  CydUi::touchAction(canvas, MINER_NEXT, ">", TFT_BLUE, false,
                     touchCapture_.activeId() == 2);
}

template <typename Canvas>
void MicriMinerApp::drawDashboard(Canvas& canvas, const MinerLogic::Stats& stats) {
  ...
  CydUi::header(canvas, pageTitle(), color, page.c_str());
  ...
  canvas.fillRect(12, 98, CydUi::W - 24, 57, TFT_BLACK);
  canvas.setTextSize(1);
  canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
  canvas.drawString(String("State ") + MinerLogic::stateLabel(stats.state), 18, 102);
  canvas.drawString(String("Pool jobs ") + String(stats.jobs), 18, 121);
  canvas.drawString(String("Shares OK ") + String(stats.accepted) + "  Rejected " + String(stats.rejected), 18, 140);
  drawPageControls(canvas, miner_.running() ? "Stop mining" : "Start mining",
                   miner_.running() ? TFT_ORANGE : TFT_GREEN);
}

template <typename Canvas>
void MicriMinerApp::drawShares(Canvas& canvas, const MinerLogic::Stats& stats) {
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_GREEN, page.c_str());
  drawInfoLine(canvas, 45, "Pool jobs", String(stats.jobs), TFT_CYAN);
  drawInfoLine(canvas, 71, "Shares out", String(stats.submitted), TFT_WHITE);
  drawInfoLine(canvas, 97, "Shares OK", String(stats.accepted), TFT_GREEN);
  drawInfoLine(canvas, 123, "Shares rej", String(stats.rejected), stats.rejected == 0 ? TFT_LIGHTGREY : TFT_RED);
  drawInfoLine(canvas, 149, "Best diff", String(stats.bestDifficulty, 6), TFT_YELLOW);
  drawPageControls(canvas, miner_.running() ? "Stop mining" : "Start mining",
                   miner_.running() ? TFT_ORANGE : TFT_GREEN);
}

template <typename Canvas>
void MicriMinerApp::drawSession(Canvas& canvas, const MinerLogic::Stats& stats) {
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_CYAN, page.c_str());
  drawInfoLine(canvas, 45, "Hashes", formatHashCount(stats.totalHashes), TFT_CYAN);
  drawInfoLine(canvas, 71, "Raw", String(static_cast<unsigned long>(stats.totalHashes / 1000ULL)) + "K", TFT_LIGHTGREY);
  drawInfoLine(canvas, 97, "Runtime", formatDuration(stats.uptimeSeconds), TFT_GREEN);
  drawInfoLine(canvas, 123, "Rate", String(stats.hashesPerSecond / 1000.0f, 1) + " KH/s", stateColor(stats.state));
  drawInfoLine(canvas, 149, "CPU temp", String(cpuTemperatureC(), 1) + " C", TFT_ORANGE);
  drawPageControls(canvas, miner_.running() ? "Stop mining" : "Start mining",
                   miner_.running() ? TFT_ORANGE : TFT_GREEN);
}

template <typename Canvas>
void MicriMinerApp::drawLifetime(Canvas& canvas, const MinerLogic::Stats& stats) {
  ...
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_ORANGE, page.c_str());
  drawInfoLine(canvas, 45, "Hashes", formatHashCount(current.hashes), TFT_ORANGE);
  drawInfoLine(canvas, 71, "Duration", formatDuration(current.seconds), TFT_GREEN);
  drawInfoLine(canvas, 97, "Best diff", String(current.bestDifficulty, 6), TFT_YELLOW);
  drawInfoLine(canvas, 123, "Jobs", String(current.jobs), TFT_CYAN);
  drawInfoLine(canvas, 149, "OK/Rej", String(current.accepted) + "/" + String(current.rejected), current.rejected == 0 ? TFT_GREEN : TFT_RED);
  drawPageControls(canvas, miner_.running() ? "Stop mining" : "Start mining",
                   miner_.running() ? TFT_ORANGE : TFT_GREEN);
}

template <typename Canvas>
void MicriMinerApp::drawPool(Canvas& canvas, const MinerLogic::Stats& stats) {
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), stateColor(stats.state), page.c_str());
  drawInfoLine(canvas, 45, "WiFi", stats.ssid[0] ? String(stats.ssid) : String("-"), TFT_CYAN);
  drawInfoLine(canvas, 71, "State", MinerLogic::stateLabel(stats.state), stateColor(stats.state));
  drawInfoLine(canvas, 97, "Pool diff", String(stats.poolDifficulty, 6), TFT_YELLOW);
  drawInfoLine(canvas, 123, "Best diff", String(stats.bestDifficulty, 6), TFT_GREEN);
  if (stats.retryInSeconds > 0) {
    drawFitAny(canvas, String("Retry ") + stats.retryCount + " in " + stats.retryInSeconds + "s", 14, 149, 292, 1, TFT_ORANGE);
  } else if (stats.lastError[0]) {
    drawFitAny(canvas, stats.lastError, 14, 149, 292, 1, TFT_RED);
  } else {
    drawInfoLine(canvas, 149, "Error", "-", TFT_LIGHTGREY);
  }
  drawPageControls(canvas, miner_.running() ? "Stop mining" : "Start mining",
                   miner_.running() ? TFT_ORANGE : TFT_GREEN);
}
...
template <typename Canvas>
void MicriMinerApp::drawPrice(Canvas& canvas) {
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_YELLOW, page.c_str());
  if (btcPriceUsd_ > 0) {
    ...
    CydUi::centered(canvas, "No price yet", 54, 2, TFT_LIGHTGREY);
  }
  drawFitAny(canvas, priceStatus_, 14, 108, 212, 1, btcPriceUsd_ > 0 ? TFT_CYAN : TFT_ORANGE);
  drawPageControls(canvas, "Refresh price", TFT_YELLOW);
}

template <typename Canvas>
void MicriMinerApp::drawSettings(Canvas& canvas) {
  settings_.load();
  const MinerLogic::Config& config = settings_.config();
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_ORANGE, page.c_str());
  drawFitAny(canvas, String("Wallet: ") + config.wallet, 14, 48, 292, 1, TFT_WHITE);
  drawFitAny(canvas, String("Pool: ") + config.poolHost + ":" + config.poolPort, 14, 77, 292, 1, TFT_CYAN);
  drawFitAny(canvas, String("Worker: ") + MinerLogic::workerName(), 14, 106, 292, 1, TFT_GREEN);
  drawFitAny(canvas, String("User: ") + MinerLogic::stratumUser(config), 14, 135, 292, 1, TFT_LIGHTGREY);
  drawPageControls(canvas, "Open setup portal", TFT_ORANGE);
}

template <typename Canvas>
void MicriMinerApp::drawReset(Canvas& canvas) {
  const String page = String(static_cast<uint8_t>(page_) + 1) + "/" + static_cast<uint8_t>(Page::Count);
  CydUi::header(canvas, pageTitle(), TFT_RED, page.c_str());
  CydUi::centered(canvas, "Reset wallet", 42, 2, TFT_WHITE);
  CydUi::centered(canvas, "and pool settings?", 68, 1, TFT_LIGHTGREY);
  drawPageControls(canvas, "Reset settings", TFT_RED);
}

template <typename Canvas>
void MicriMinerApp::drawPortal(Canvas& canvas) {
  CydUi::header(canvas, mode_ == Mode::StartingPortal ? "Miner Setup" : "Setup AP", TFT_ORANGE);
  if (mode_ == Mode::StartingPortal) {
    CydUi::centered(canvas, "Launching", 46, 2, TFT_YELLOW);
    CydUi::centered(canvas, "Miner Setup AP", 73, 2, TFT_ORANGE);
  } else {
    CydUi::centered(canvas, MinerLogic::SETUP_AP_SSID, 42, 2, TFT_ORANGE);
    CydUi::centered(canvas, "Pass: micriminer", 70, 1, TFT_LIGHTGREY);
    CydUi::centered(canvas, "Open 192.168.4.1", 88, 1, TFT_CYAN);
  }
  CydUi::touchAction(canvas, MINER_CONTINUE, "Back", TFT_LIGHTGREY, false,
                     touchCapture_.activeId() == 0);
}

template <typename Canvas>
void MicriMinerApp::drawMessage(Canvas& canvas) {
  CydUi::header(canvas, "Micri Miner", TFT_YELLOW);
  CydUi::largeValue(canvas, message_, 54, TFT_YELLOW);
  CydUi::touchAction(canvas, MINER_CONTINUE, "Continue", TFT_GREEN, true,
                     touchCapture_.activeId() == 0);
}

template <typename Canvas>
void MicriMinerApp::drawConfirmReset(Canvas& canvas) {
  CydUi::confirmation(canvas, "Reset Miner", "Restore wallet and pool defaults?",
                      MINER_CANCEL, MINER_CONFIRM, "Reset");
}

template <typename Canvas>
void MicriMinerApp::drawFrame(Canvas& canvas) {
  CydUi::clear(canvas);
  if (mode_ == Mode::StartingPortal || mode_ == Mode::Portal) {
    drawPortal(canvas);
    return;
...
void MicriMinerApp::drawRunning(TFT_eSPI& tft) {
...
void MicriMinerApp::drawStart(TFT_eSPI& tft) {
  CydHardware::applyDisplayOrientation(tft);
  CydUi::clear(tft);
  if (showStartPromptPage()) {
    CydUi::header(tft, "Micri Miner", TFT_ORANGE);
    CydUi::centered(tft, "Press", 50, 3, TFT_WHITE);
    CydUi::centered(tft, "to Start", 80, 2, TFT_LIGHTGREY);
    CydUi::footer(tft, "Tap Start Mining");
  } else if (showStartScorePage()) {
    CydUi::header(tft, "Solo Miner", TFT_YELLOW);
    CydUi::centered(tft, "public-pool.io", 49, 2, TFT_CYAN);
    CydUi::centered(tft, "Wallet configurable", 78, 1, TFT_LIGHTGREY);
    CydUi::footer(tft, "Settings via portal");
  } else {
    CydUi::header(tft, "Micri Miner", TFT_ORANGE);
