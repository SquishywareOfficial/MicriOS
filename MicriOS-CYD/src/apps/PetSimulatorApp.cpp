...
const TouchUi::GridMetrics PET_GRID = {{12, 42, 296, 42}, 8, 8, 4, 1};
constexpr TouchUi::Rect ADOPT_BUTTON = {88, 163, 144, 34};
const TouchUi::GridMetrics ACTION_GRID = {{126, 45, 182, 142}, 7, 7, 2, 3};
constexpr TouchUi::Rect PET_BACK = {88, 165, 144, 34};
constexpr TouchUi::Rect PET_CONTINUE = {88, 158, 144, 38};
...
bool PetSimulatorApp::handleTouch(const TouchUi::TouchSample& sample,
  ...
  if (mode_ == Mode::ChoosePet) {
    hit = TouchUi::cardAt(PET_GRID, PET_COUNT, 0, point);
    if (hit == TouchUi::NO_CONTROL && ADOPT_BUTTON.contains(point)) hit = 4;
  } else if (mode_ == Mode::Menu) {
    const int16_t action = TouchUi::cardAt(ACTION_GRID, ACTION_COUNT, 0, point);
    if (action >= 0) hit = 10 + action;
  } else if (mode_ == Mode::Stats) {
    if (PET_BACK.contains(point)) hit = 30;
  } else if (mode_ == Mode::Message || mode_ == Mode::PlayScene) {
    if (PET_CONTINUE.contains(point)) hit = 31;
  } else if (mode_ == Mode::Dead) {
    if (PET_CONTINUE.contains(point)) hit = 32;
  }

  const TouchUi::CaptureResult result = touchCapture_.update(sample, event, hit);
  if (!result.activated) return result.consumed;
  idleMs_ = 0;
  if (result.id >= 0 && result.id < PET_COUNT) {
    petType_ = static_cast<uint8_t>(result.id);
  } else if (result.id == 4) {
    resetCareStats();
    savePet();
    mode_ = Mode::Menu;
  } else if (result.id >= 10 && result.id < 10 + ACTION_COUNT) {
    menuIndex_ = static_cast<uint8_t>(result.id - 10);
    applyAction();
  } else if (result.id == 30 || result.id == 31) {
    mode_ = Mode::Menu;
  } else if (result.id == 32) {
    resetCareStats();
    loaded_ = false;
...
void PetSimulatorApp::drawBar(Canvas& tft, int x, int y, const char* label, uint8_t value) {
  ...
  tft.drawRect(x + 42, y - 1, 190, 10, TFT_WHITE);
  const uint16_t color = value > 60 ? TFT_GREEN : value > 25 ? TFT_YELLOW : TFT_RED;
  tft.fillRect(x + 44, y + 1, map(value, 0, 100, 0, 186), 6, color);
}
...
void PetSimulatorApp::drawRunning(TFT_eSPI& tft) {
  ...
  if (idleMode_) {
    ...
    drawFooter(canvas, width, height, "Tap to wake");
    return;
  }
  ...
  if (mode_ == Mode::ChoosePet) {
    ...
    for (uint8_t i = 0; i < PET_COUNT; ++i) {
      CydUi::touchAction(canvas, PET_GRID.cardRect(i), PET_NAMES[i],
                         i == petType_ ? TFT_CYAN : TFT_LIGHTGREY,
                         i == petType_);
    }
    drawPet(canvas, width / 2, 125);
    CydUi::touchAction(canvas, ADOPT_BUTTON, "Adopt", TFT_GREEN);
    return;
  }
  ...
  if (mode_ == Mode::Dead) {
    ...
    canvas.drawString("RIP", 122, 65);
    canvas.setTextSize(2);
    canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    canvas.drawString(PET_NAMES[petType_], 126, 127);
    CydUi::touchAction(canvas, PET_CONTINUE, "Choose New Pet", TFT_GREEN);
    return;
  }
  ...
  if (mode_ == Mode::PlayScene) {
    ...
    drawPet(canvas, petX_, petY_);
    CydUi::touchAction(canvas, PET_CONTINUE, "Done", TFT_CYAN, false);
    return;
  }

  if (mode_ == Mode::Stats) {
    drawShell(canvas, width, height, "Pet Stats");
    drawBar(canvas, 24, 50, "Hunger", hunger_);
    drawBar(canvas, 24, 79, "Fun", fun_);
    drawBar(canvas, 24, 108, "Energy", energy_);
    drawBar(canvas, 24, 137, "Clean", clean_);
    canvas.setTextSize(1);
    canvas.setTextColor(TFT_WHITE, TFT_BLACK);
    canvas.setCursor(270, 160);
    canvas.print("HP ");
    canvas.print(health_);
    drawLowHealthWarning(canvas);
    CydUi::touchAction(canvas, PET_BACK, "Back", TFT_CYAN, false);
    return;
  }
  ...
  if (mode_ == Mode::Message) {
    ...
    drawPoops(canvas);
    drawPet(canvas, 88, 111);
    canvas.setTextSize(3);
    canvas.setTextColor(TFT_GREEN, TFT_BLACK);
    canvas.drawString(message_, 160, 91);
    drawLowHealthWarning(canvas);
    CydUi::touchAction(canvas, PET_CONTINUE, "Continue", TFT_GREEN);
    return;
  }
  ...
  for (uint8_t i = 0; i < ACTION_COUNT; ++i) {
    const uint16_t color = i == 5 ? TFT_RED : (i == 4 ? TFT_CYAN : TFT_GREEN);
    CydUi::touchAction(canvas, ACTION_GRID.cardRect(i), ACTIONS[i], color, i < 4);
  }
  drawLowHealthWarning(canvas);
  });
}
