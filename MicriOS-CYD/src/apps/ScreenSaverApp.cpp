...
namespace {
constexpr uint8_t SAVERS_PER_PAGE = 6;
const TouchUi::GridMetrics SAVER_GRID = {{10, 40, 300, 126}, 8, 8, 3, 2};
constexpr TouchUi::Rect SAVER_PREV = {12, 174, 92, 27};
constexpr TouchUi::Rect SAVER_NEXT = {216, 174, 92, 27};
...
bool ScreenSaverApp::prefersImmersiveMode() const {
...
bool ScreenSaverApp::handleTouch(const TouchUi::TouchSample& sample,
                                 const TouchUi::TouchEvent& event) {
  if (mode_ == Mode::Play) {
    if (event.swipe == TouchUi::SwipeDirection::Left ||
        event.swipe == TouchUi::SwipeDirection::Right) {
      const int8_t direction = event.swipe == TouchUi::SwipeDirection::Left ? 1 : -1;
      int16_t next = static_cast<int16_t>(selected_) + direction;
      if (next < 0) next = static_cast<int16_t>(Saver::Count) - 1;
      if (next >= static_cast<int16_t>(Saver::Count)) next = 0;
  ...
  const uint8_t count = static_cast<uint8_t>(Saver::Count);
  const uint8_t pages = TouchUi::pageCount(count, SAVERS_PER_PAGE);
  if (event.swipe == TouchUi::SwipeDirection::Left && selectPage_ + 1 < pages) {
    selectPage_++;
    dirty_ = true;
    touchCapture_.reset();
    return true;
  }
  if (event.swipe == TouchUi::SwipeDirection::Right && selectPage_ > 0) {
    selectPage_--;
    dirty_ = true;
    touchCapture_.reset();
    return true;
  ...
  int16_t hit = TouchUi::cardAt(SAVER_GRID, count,
                                TouchUi::pageOffset(selectPage_, SAVERS_PER_PAGE),
                                point);
  if (hit == TouchUi::NO_CONTROL && SAVER_PREV.contains(point)) hit = 30;
  if (hit == TouchUi::NO_CONTROL && SAVER_NEXT.contains(point)) hit = 31;
  const TouchUi::CaptureResult result = touchCapture_.update(sample, event, hit);
  if (!result.activated) return result.consumed;
  if (result.id >= 0 && result.id < count) {
    selected_ = static_cast<Saver>(result.id);
    mode_ = Mode::Play;
    resetAnimation();
  } else if (result.id == 30 && selectPage_ > 0) {
    selectPage_--;
    dirty_ = true;
  } else if (result.id == 31 && selectPage_ + 1 < pages) {
    selectPage_++;
    dirty_ = true;
  }
  return true;
...
void ScreenSaverApp::drawSelect(TFT_eSPI& tft) {
  ...
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "Screen Savers", TFT_CYAN);
    const uint8_t count = static_cast<uint8_t>(Saver::Count);
    const uint8_t offset = TouchUi::pageOffset(selectPage_, SAVERS_PER_PAGE);
    for (uint8_t slot = 0; slot < SAVER_GRID.capacity(); ++slot) {
      const uint8_t index = offset + slot;
      if (index >= count) break;
      CydUi::touchAction(canvas, SAVER_GRID.cardRect(slot),
                         saverName(static_cast<Saver>(index)), rainbow(index), false);
    }
    const uint8_t pages = TouchUi::pageCount(count, SAVERS_PER_PAGE);
    CydUi::touchAction(canvas, SAVER_PREV, "Previous", TFT_LIGHTGREY, false);
    CydUi::touchAction(canvas, SAVER_NEXT, "Next", TFT_CYAN, false);
    char page[12];
    snprintf(page, sizeof(page), "%u / %u", selectPage_ + 1, pages);
    CydUi::centered(canvas, page, 181, 1, TFT_LIGHTGREY);
  });
  dirty_ = false;
}
...
void ScreenSaverApp::drawText(Canvas& canvas) {
  ...
  canvas.drawString("MicriOS", textX_ + 5, textY_ + 5);
  canvas.setTextColor(TFT_DARKGREY, TFT_BLACK);
  canvas.drawString("MicriOS", textX_ + 3, textY_ + 3);
  canvas.setTextColor(rainbow(frame_ / 10), TFT_BLACK);
  canvas.drawString("MicriOS", textX_, textY_);
}
...
void ScreenSaverApp::drawNightDrive(Canvas& canvas) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    const int y = horizon + 7 + ((i * 29 + frame_ / 2) % 82);
  ...
  canvas.fillRect(0, height - 43, width, 43, TFT_BLACK);
  canvas.fillRoundRect(58, height - 34, 82, 28, 8, rgb565(0, 18, 28));
  canvas.fillRoundRect(155, height - 36, 92, 30, 8, rgb565(13, 8, 21));
  canvas.drawFastHLine(0, height - 43, width, rgb565(0, 55, 80));
  canvas.drawFastHLine(0, height - 42, width, rgb565(80, 0, 70));
  canvas.drawCircle(35, height - 17, 27, rgb565(0, 80, 120));
  canvas.drawFastHLine(164, height - 21, 68, TFT_CYAN);
  canvas.drawPixel(198 + (frame_ % 16), height - 27, TFT_RED);
}
