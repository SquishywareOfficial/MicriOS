...
constexpr const char* AP_SSID = "MicriDeck-Cyd";
constexpr const char* AP_PASS = "micrideck";
...
constexpr uint8_t PROFILE_ROWS = 4;
constexpr TouchUi::Rect PRIMARY_ACTION = {168, 151, 132, 43};
constexpr TouchUi::Rect SECONDARY_ACTION = {20, 151, 132, 43};
constexpr TouchUi::Rect CENTER_ACTION = {90, 151, 140, 43};
constexpr TouchUi::Rect PAGE_PREV = {12, 174, 76, 28};
constexpr TouchUi::Rect PAGE_NEXT = {232, 174, 76, 28};

TouchUi::Rect wifiRow(uint8_t row) {
  return {14, static_cast<int16_t>(43 + row * 38), 292, 34};
}

TouchUi::Rect profileRow(uint8_t row) {
...
bool WiFiSetupApp::handleTouch(const TouchUi::TouchSample& sample,
  ...
  if (mode_ == Mode::Main) {
    for (uint8_t i = 0; i < MAIN_COUNT; ++i) {
      if (wifiRow(i).contains(point)) hit = i;
    }
  } else if (mode_ == Mode::StartingPortal || mode_ == Mode::Message) {
    if (CENTER_ACTION.contains(point)) hit = 0;
  } else if (mode_ == Mode::ConfigurePortal || mode_ == Mode::Testing ||
             mode_ == Mode::ConfirmDelete) {
    if (SECONDARY_ACTION.contains(point)) hit = 0;
    if (PRIMARY_ACTION.contains(point)) hit = 1;
  } else if (mode_ == Mode::TestSelect || mode_ == Mode::DeleteSelect) {
    const uint8_t count = profileMenuCount();
    for (uint8_t row = 0; row < PROFILE_ROWS; ++row) {
      const uint8_t index = profilePage_ * PROFILE_ROWS + row;
      if (index < count && profileRow(row).contains(point)) hit = row;
    }
  ...
  if (mode_ == Mode::Main) {
    ...
    if (result.id == 0) {
      ...
      if (logic_.getProfileCount() == 0) {
        ...
        profileMenuIndex_ = 0;
        profilePage_ = 0;
        mode_ = result.id == 1 ? Mode::TestSelect : Mode::DeleteSelect;
      }
    } else {
      stopPortal();
  ...
  } else if (mode_ == Mode::StartingPortal) {
    ...
    const uint8_t count = profileMenuCount();
    const uint8_t pageCount = (count + PROFILE_ROWS - 1) / PROFILE_ROWS;
    if (result.id < PROFILE_ROWS) {
      profileMenuIndex_ = profilePage_ * PROFILE_ROWS + result.id;
      const uint8_t profileCount = logic_.getProfileCount();
      if (profileMenuIndex_ == profileCount + 1) {
        mode_ = Mode::Main;
        ...
        if (profileMenuIndex_ == profileCount) {
          ...
          if (slot >= 0) beginProfileTest(slot, false);
        }
        mode_ = Mode::Testing;
      } else {
        if (profileMenuIndex_ == profileCount) {
          ...
          const int8_t slot = profileSlotForPosition(profileMenuIndex_);
          deleteAll_ = false;
          deleteProfileIndex_ = slot < 0 ? NO_PROFILE : slot;
        }
        mode_ = Mode::ConfirmDelete;
      }
    } else if (result.id == 4) {
      profilePage_ = profilePage_ == 0 ? pageCount - 1 : profilePage_ - 1;
    } else if (result.id == 5) {
      profilePage_ = (profilePage_ + 1) % pageCount;
    }
  } else if (mode_ == Mode::Testing) {
    if (result.id == 0) {
      stopStationTest(true);
      mode_ = Mode::TestSelect;
    } else if (testState_ != TestState::Connecting) {
      stopStationTest(true);
      mode_ = Mode::Main;
    }
...
String WiFiSetupApp::buildPage(const char* notice) const {
  ...
  html += F("<title>MicriOS WiFi</title><style>");
  ...
  html += F("<h1>MicriOS WiFi Setup</h1>");
  html += F("<p class='muted'>Save up to five WiFi networks. Setup WiFi password is <strong>micrideck</strong>. Testing and deleting saved networks is done from MicriOS.</p>");
  ...
  html += F("<p class='muted'>After saving, return to MicriOS and use Back on the device.</p>");
...
void WiFiSetupApp::drawRunning(TFT_eSPI& tft) {
  ...
  switch (mode_) {
    ...
    case Mode::TestSelect:
      drawProfileSelect(tft, "Test WiFi", TFT_CYAN, "");
      break;
    case Mode::Testing:
      drawTesting(tft);
      break;
    case Mode::DeleteSelect:
      drawProfileSelect(tft, "Delete WiFi", TFT_RED, "");
      break;
...
void WiFiSetupApp::drawMain(TFT_eSPI& tft) {
  ...
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "WiFi Setup", TFT_CYAN, saved.c_str());
    for (uint8_t i = 0; i < MAIN_COUNT; ++i) {
      const char* detail = i == 0 ? "Portal" : i == 1 ? "Test" :
                           i == 2 ? "Delete" : nullptr;
      CydUi::touchListRow(canvas, wifiRow(i), MAIN_ITEMS[i],
                          i == mainIndex_, i == 2 ? TFT_RED : TFT_CYAN,
                          detail, touchCapture_.activeId() == i);
    }
  });
}
...
void WiFiSetupApp::drawStartingPortal(TFT_eSPI& tft) {
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "WiFi Setup", TFT_CYAN, "Starting");
    CydUi::centered(canvas, "Launching", 45, 2, TFT_YELLOW);
    CydUi::centered(canvas, "WiFi AP", 69, 2, TFT_CYAN);
    canvas.setTextDatum(TL_DATUM);
    canvas.setTextSize(1);
    canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    canvas.drawString("Scanning networks first...", 44, 102);
    CydUi::touchAction(canvas, CENTER_ACTION, "Cancel", TFT_LIGHTGREY, false,
...
void WiFiSetupApp::drawConfigure(TFT_eSPI& tft) {
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "Configure", portalState_ == PortalState::Error ? TFT_RED : TFT_CYAN, portalLabel());
    canvas.setTextDatum(TL_DATUM);
    canvas.setTextSize(1);
    canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    canvas.drawString("Connect to setup WiFi:", 14, 38);
    CydUi::centered(canvas, AP_SSID, 54, 2, TFT_CYAN);
    canvas.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    canvas.drawString("Pass: micrideck", 14, 84);
    canvas.drawString("Open: 192.168.4.1", 14, 98);
    CydUi::touchAction(canvas, SECONDARY_ACTION, "Back", TFT_LIGHTGREY,
                       false, touchCapture_.activeId() == 0);
    CydUi::touchAction(canvas, PRIMARY_ACTION, "Rescan", TFT_CYAN, true,
...
void WiFiSetupApp::drawProfileSelect(TFT_eSPI& tft, const char* title, uint16_t color, const char* action) {
  ...
  const uint8_t pageCount = (count + PROFILE_ROWS - 1) / PROFILE_ROWS;
  drawBuffered(tft, width, height, [&](auto& canvas) {
    String page = String(profilePage_ + 1) + "/" + String(pageCount);
    CydUi::header(canvas, title, color, page.c_str());
    for (uint8_t row = 0; row < PROFILE_ROWS; row++) {
      const uint8_t item = profilePage_ * PROFILE_ROWS + row;
      if (item >= count) break;
      String label;
      const uint8_t profileCount = logic_.getProfileCount();
      ...
      const char* detail = item == profileCount + 1 ? "Back" :
          (mode_ == Mode::TestSelect ? "Test" : "Delete");
      CydUi::touchListRow(canvas, profileRow(row), label.c_str(),
                          item == profileMenuIndex_, color, detail,
...
void WiFiSetupApp::drawTesting(TFT_eSPI& tft) {
  ...
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "WiFi Test", color, testLabel());
    if (testProfileIndex_ != NO_PROFILE && testProfileIndex_ < WiFiLogic::MAX_PROFILES) {
      CydUi::centered(canvas, logic_.getSsid(testProfileIndex_), 42, 2, TFT_WHITE);
    }
    if (ok) {
      CydUi::centered(canvas, WiFi.localIP().toString(), 79, 2, TFT_GREEN);
    } else if (fail) {
      CydUi::centered(canvas, "No connection", 79, 2, TFT_RED);
    } else {
      CydUi::centered(canvas, "Connecting...", 79, 2, TFT_YELLOW);
    }
    CydUi::touchAction(canvas, SECONDARY_ACTION,
                       testState_ == TestState::Connecting ? "Cancel" : "List",
                       TFT_LIGHTGREY, false,
                       touchCapture_.activeId() == 0);
    if (testState_ != TestState::Connecting) {
      CydUi::touchAction(canvas, PRIMARY_ACTION, "Done", TFT_GREEN, true,
                         touchCapture_.activeId() == 1);
    }
  });
}
...
void WiFiSetupApp::drawConfirmDelete(TFT_eSPI& tft) {
  drawBuffered(tft, width, height, [&](auto& canvas) {
    String message = deleteAll_ ? "Delete every saved WiFi network?" :
        (deleteProfileIndex_ < WiFiLogic::MAX_PROFILES
             ? String("Delete ") + logic_.getSsid(deleteProfileIndex_) + "?"
             : String("Delete this WiFi network?"));
    CydUi::confirmation(canvas, "Delete WiFi", message, SECONDARY_ACTION,
...
void WiFiSetupApp::drawMessage(TFT_eSPI& tft) {
  drawBuffered(tft, width, height, [&](auto& canvas) {
    CydUi::header(canvas, "WiFi Setup", TFT_CYAN);
    CydUi::largeValue(canvas, message_, 52, TFT_YELLOW);
    CydUi::touchAction(canvas, CENTER_ACTION, "Continue", TFT_GREEN, true,
                       touchCapture_.activeId() == 0);
  });
}
