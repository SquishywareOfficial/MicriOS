...
namespace {
constexpr TouchUi::Rect SEND_TAB = {12, 38, 145, 27};
constexpr TouchUi::Rect INBOX_TAB = {163, 38, 145, 27};
constexpr TouchUi::Rect COMM_PREV = {12, 176, 72, 27};
constexpr TouchUi::Rect COMM_BACK = {91, 176, 138, 27};
constexpr TouchUi::Rect COMM_NEXT = {236, 176, 72, 27};
constexpr TouchUi::Rect MESSAGE_BACK = {90, 158, 140, 40};

TouchUi::Rect commRow(uint8_t row) {
  return {12, static_cast<int16_t>(69 + row * 26), 296, 24};
}
}

bool CommunicatorApp::handleTouch(const TouchUi::TouchSample& sample,
                                  const TouchUi::TouchEvent& event) {
  const TouchUi::Point point = TouchUi::currentPoint(sample, event);
  int16_t hit = TouchUi::NO_CONTROL;
  const bool listMode = mode_ == UiMode::Send || mode_ == UiMode::Recipient ||
                        mode_ == UiMode::Inbox;
  if (listMode) {
    if (SEND_TAB.contains(point)) hit = 10;
    if (INBOX_TAB.contains(point)) hit = 11;
    if (COMM_PREV.contains(point)) hit = 4;
    if (COMM_BACK.contains(point)) hit = 5;
    if (COMM_NEXT.contains(point)) hit = 6;
    for (uint8_t row = 0; row < ROW_COUNT; ++row) {
      if (commRow(row).contains(point)) hit = row;
    }
  } else if (MESSAGE_BACK.contains(point)) {
    hit = 0;
  }
  ...
  if (result.id == 10) {
    ...
    inboxIndex_ = 0;
    inboxPage_ = 0;
  } else if (mode_ == UiMode::Send) {
    uint8_t count = 0;
    currentList(count);
    if (depth_ == 0) ++count;
    const uint8_t pages = count == 0 ? 1 : (count + ROW_COUNT - 1) / ROW_COUNT;
    if (result.id < ROW_COUNT) {
      const uint8_t item = sendPage_ * ROW_COUNT + result.id;
      if (item < count) {
        index_ = item;
        handleHold();
        if (mode_ == UiMode::Send) sendPage_ = 0;
        if (mode_ == UiMode::Recipient) recipientPage_ = 0;
      }
    } else if (result.id == 4) {
      sendPage_ = sendPage_ == 0 ? pages - 1 : sendPage_ - 1;
    } else if (result.id == 5) {
      handleBack();
      sendPage_ = 0;
    } else if (result.id == 6) {
      sendPage_ = (sendPage_ + 1) % pages;
    }
  } else if (mode_ == UiMode::Recipient) {
    const uint8_t count = contactBook_.count() + 1;
    const uint8_t pages = (count + ROW_COUNT - 1) / ROW_COUNT;
    if (result.id < ROW_COUNT) {
      const uint8_t item = recipientPage_ * ROW_COUNT + result.id;
      if (item < count) {
        recipientIndex_ = item;
        handleHold();
      }
    } else if (result.id == 4) {
      recipientPage_ = recipientPage_ == 0 ? pages - 1 : recipientPage_ - 1;
    } else if (result.id == 5) {
      handleBack();
    } else if (result.id == 6) {
      recipientPage_ = (recipientPage_ + 1) % pages;
    }
  } else if (mode_ == UiMode::Inbox) {
    const uint8_t count = logic_.getInboxCount() + 1;
    const uint8_t pages = (count + ROW_COUNT - 1) / ROW_COUNT;
    if (result.id < ROW_COUNT) {
      const uint8_t item = inboxPage_ * ROW_COUNT + result.id;
      if (item < count) {
        inboxIndex_ = item;
        handleHold();
      }
    } else if (result.id == 4) {
      inboxPage_ = inboxPage_ == 0 ? pages - 1 : inboxPage_ - 1;
    } else if (result.id == 5) {
      mode_ = UiMode::Send;
    } else if (result.id == 6) {
      inboxPage_ = (inboxPage_ + 1) % pages;
    }
  } else if (mode_ == UiMode::OpenMessage) {
    mode_ = UiMode::Inbox;
    openInboxIndex_ = NO_MESSAGE;
  } else if (mode_ == UiMode::Feedback) {
    mode_ = returnMode_;
  }
  touchCapture_.reset();
  markDirty();
...
void CommunicatorApp::onAppExit() {
  shutdownRadio();
}
...
void CommunicatorApp::drawStart(TFT_eSPI& tft) {
  ...
  CydUi::clear(tft);
  CydUi::header(tft, "Communicator", TFT_CYAN);
  CydUi::centered(tft, "ESP-NOW", 47, 3, TFT_WHITE);
  CydUi::centered(tft, "peer messages", 80, 2, TFT_LIGHTGREY);
  CydUi::footer(tft, "Tap Send or Inbox");
  startDirty_ = false;
}

template <typename Canvas>
void CommunicatorApp::drawSend(Canvas& tft) {
  CydUi::header(tft, currentTitle(), radioReady_ ? TFT_CYAN : TFT_RED, radioReady_ ? "radio" : "!");
  CydUi::touchTab(tft, SEND_TAB, depth_ == 0 ? "Send" : currentTitle(), true,
                  TFT_CYAN, touchCapture_.activeId() == 10);
  CydUi::touchTab(tft, INBOX_TAB, "Inbox", false, TFT_YELLOW,
                  touchCapture_.activeId() == 11);
  uint8_t count = 0;
  const Node* list = currentList(count);
  if (depth_ == 0) {
    count++;
  ...
  const uint8_t pages = count == 0 ? 1 : (count + ROW_COUNT - 1) / ROW_COUNT;
  for (uint8_t row = 0; row < ROW_COUNT; row++) {
    const uint8_t item = sendPage_ * ROW_COUNT + row;
    if (item >= count) break;
    ...
    CydUi::touchListRow(tft, commRow(row), label, item == index_, TFT_CYAN,
                        "Open", touchCapture_.activeId() == row);
  }
  CydUi::touchAction(tft, COMM_PREV, "Prev", TFT_BLUE, false,
                     touchCapture_.activeId() == 4);
  CydUi::touchAction(tft, COMM_BACK, depth_ == 0 ? "Inbox" : "Back",
                     TFT_LIGHTGREY, false, touchCapture_.activeId() == 5);
  CydUi::touchAction(tft, COMM_NEXT, "Next", TFT_BLUE, false,
...
void CommunicatorApp::drawRecipient(Canvas& tft) {
  ...
  CydUi::header(tft, "Send To", TFT_CYAN, message.c_str());
  CydUi::touchTab(tft, SEND_TAB, "Send", true, TFT_CYAN,
                  touchCapture_.activeId() == 10);
  CydUi::touchTab(tft, INBOX_TAB, "Inbox", false, TFT_YELLOW,
                  touchCapture_.activeId() == 11);
  const uint8_t count = contactBook_.count() + 1;
  const uint8_t pages = (count + ROW_COUNT - 1) / ROW_COUNT;
  for (uint8_t row = 0; row < ROW_COUNT; row++) {
    const uint8_t item = recipientPage_ * ROW_COUNT + row;
    if (item >= count) break;
    CydUi::touchListRow(tft, commRow(row), recipientLabel(item),
                        item == recipientIndex_, TFT_CYAN, "Send",
                        touchCapture_.activeId() == row);
  }
  CydUi::touchAction(tft, COMM_PREV, "Prev", TFT_BLUE, false,
                     touchCapture_.activeId() == 4);
  CydUi::touchAction(tft, COMM_BACK, "Back", TFT_LIGHTGREY, false,
...
void CommunicatorApp::drawInbox(Canvas& tft) {
  ...
  CydUi::header(tft, "Inbox", TFT_YELLOW, stat.c_str());
  CydUi::touchTab(tft, SEND_TAB, "Send", false, TFT_CYAN,
                  touchCapture_.activeId() == 10);
  CydUi::touchTab(tft, INBOX_TAB, "Inbox", true, TFT_YELLOW,
                  touchCapture_.activeId() == 11);
  const uint8_t count = logic_.getInboxCount() + 1;
  const uint8_t pages = (count + ROW_COUNT - 1) / ROW_COUNT;
  for (uint8_t row = 0; row < ROW_COUNT; row++) {
    const uint8_t item = inboxPage_ * ROW_COUNT + row;
    if (item >= count) break;
    String label;
    ...
    CydUi::touchListRow(tft, commRow(row), label.c_str(),
                        item == inboxIndex_, TFT_YELLOW,
                        item >= logic_.getInboxCount() ? "Send" : "Open",
                        touchCapture_.activeId() == row);
  }
  CydUi::touchAction(tft, COMM_PREV, "Prev", TFT_BLUE, false,
                     touchCapture_.activeId() == 4);
  CydUi::touchAction(tft, COMM_BACK, "Send", TFT_LIGHTGREY, false,
                     touchCapture_.activeId() == 5);
  CydUi::touchAction(tft, COMM_NEXT, "Next", TFT_BLUE, false,
                     touchCapture_.activeId() == 6);
}
...
void CommunicatorApp::drawOpenMessage(Canvas& tft) {
  CydUi::header(tft, "Message", TFT_YELLOW);
  if (openInboxIndex_ < logic_.getInboxCount()) {
    ...
    CydUi::centered(tft, route, 49, 2, TFT_LIGHTGREY);
    CydUi::largeValue(tft, packetMessageText(inbox.packet), 83, TFT_WHITE);
    String mac;
    ...
    CydUi::centered(tft, (logic_.firmwareMismatch(inbox.packet) ? "v! " : "") + String("from ...") + mac, 151, 1, TFT_LIGHTGREY);
  }
  CydUi::touchAction(tft, MESSAGE_BACK, "Back to inbox", TFT_YELLOW, false,
                     touchCapture_.activeId() == 0);
}

template <typename Canvas>
void CommunicatorApp::drawFeedback(Canvas& tft) {
  CydUi::header(tft, "Communicator", feedbackColor_);
  CydUi::largeValue(tft, feedbackText_, 55, feedbackColor_);
  CydUi::touchAction(tft, MESSAGE_BACK, "Continue", TFT_CYAN, true,
                     touchCapture_.activeId() == 0);
}
